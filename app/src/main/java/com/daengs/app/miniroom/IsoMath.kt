...
object RoomSpec {
     ...
     * 그대로 드러나서 어색해졌다.
     *
     * 바닥 앞 모서리는 그림 높이의 94.5% 지점이다([FloorQuad.front]). 5.5% 까지는
     * 굽도리만 없어지고 바닥은 한 줄도 안 잘린다. 지금 값(7%)은 그보다 조금 더 내려서
     * **바닥 앞 꼭짓점의 뾰족한 끝만** 살짝 가린다 — 그 지점은 격자의 맨 앞 모서리라
     * 물건을 놓아도 거의 안 쓰이는 자리다.
     *
     * 10% 를 넘기면 앞줄(row = GRID - 1)이 눈에 띄게 잘리기 시작하므로 거기가 상한이다.
     */
    const val BOTTOM_BLEED = 0.07f
}
